...
const SongPlayerModal = ({ song, type, open, onClose }: SongPlayerModalProps) => {
  ...
  // We DON'T use a timeout — we only show the fallback when YouTube
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="p-0 gap-0 overflow-hidden max-w-lg border-border/50 bg-card [&>button]:hidden">
        ...
        {embedBlocked ? (
          ...
          /* Fallback — shown ONLY when YouTube signals the video can't be embedded */
          ...
          /* ── Audio mode — iframe plays audio; album-art overlay hides the video ── */
          <div className="relative w-full bg-black" style={{ aspectRatio: "16/9" }}>
            ...
            {/* Overlay — covers the video, shows album art + animated equaliser */}
              ...
              {thumbnail && (
                <div className="relative w-36 h-36 rounded-2xl overflow-hidden shadow-2xl ring-2 ring-primary/30">
                  <img
                    ...
                    className="w-full h-full object-cover"
                  />
                  {/* Animated equaliser — shows once player is ready */}
                  {playerReady && (
